...
- **Not tabs** (reached from the Översikt footer / cards; breadcrumb parent `/bokforing`): `/bokforing/periodstangning` (*Periodstängning och periodlås*), `/bokforing/ingaende-balans` (*Ingående balans och övertagande från annat program*; also the card *Kommer bokföringen från ett annat program?* → **Ingående balans**, shown only for an open year with empty IB and zero verifications). Legacy `/bokforing/detaljer` → verifikationer.
...
- **Overview copy:** *Sköts automatiskt i bakgrunden – du behöver bara svara när något är oklart.* Headline *N bokföringsfrågor att lösa*. Section **Behöver lösas**. Cards *Kommande* (moms), *Företaget i år*, *Bokslutet närmar sig* (from 2 months before year end), *Kommer bokföringen från ett annat program?* (empty start only), *Nyligen bokfört* (when all good). Footer **Bokföringsdetaljer**: *Visa bokföringsdetaljer*, *Periodstängning och periodlås*, *Ingående balans och övertagande från annat program*.
...
- Not a tab: Översikt footer **Periodstängning och periodlås**; attention `period-close-<YYYY-MM>` (*{månad} är klar att stängas*, CTA **Öppna periodstängningen**) only when the next month has **no blockers and ≥ 1 verification** - empty months are skipped in the queue (`collectPeriodClose`) but still listed on the page.
