...
- **Main actions — godkänd/avböjd:** Ny version (new version → utkast) · Öppna kundvyn. Godkänd shows a **compact** acceptance banner only: *Godkänd av {namn}*, date/time, **Visa intyg** (`/offert/[token]/underlag`). No IP, user-agent, SHA-256, method essay, emails, or “Version N är låst…” paragraph in that card. Header may still show a small *Version N låst* badge. Forensics live on the intyg page / PDF.
