...
- **Priority:** bookkeeping=existing → *Flytta in bokföringen* first; consultant → *Bjud in din redovisningskonsult* first; new → customer/job; payment details jump first once quotes/invoices exist. Optional (non-recommended) tasks never show on Hem. Card disappears when no recommended task is open; center stays reachable.
