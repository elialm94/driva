...
- **Receipt for a specific purchase (file is stored):** CTA **Lägg till kvitto** on the *Kvitto saknas – {leverantör}, {belopp}* attention item (Hem / Bokföring, `attention-list.tsx`, `cta.type === "uploadReceipt"`) or `UploadReceiptButton expenseId=…`. Client sends the `File` in a `FormData` (`receiptUploadForm`, `src/lib/receipts/read-file.ts`, max **5 MB** → *Kvittot är för stort (max 5 MB).*) — **not** a data-URL string: React Flight caps string action arguments at ~1e6 chars, so a data URL for anything over ~730 KB failed in decoding with a raw Next error → `uploadReceiptAction(form)` → `receiptFileFromForm` + `validateReceiptFile` (PDF/JPEG/PNG/WebP/HEIC, else *Kvittot måste vara en bild (JPEG/PNG/WebP/HEIC) eller PDF.*; empty browser type falls back to the extension). **Order inside `withBusiness`:** `expenseAwaitingReceipt` (purchase exists, has no receipt) → `storeReceiptFile` against a pre-generated receipt id → `uploadReceiptForExpense(…, { id, …stored })` → assert `receiptFileStored(receipt)`. The file is stored **before** any state mutation: `uploadReceiptForExpense` calls `save()` and books the purchase, and a demo session persists dirty state in `finally` even when the action throws — storing first is what guarantees a failed upload leaves *Saknar kvitto* in place (attention item stays, register row stays *Saknar kvitto*, no receipt row). `ok: true` **always** means a stored file (`fileStored: true`, `receiptId`); a filename-only receipt is never a success on this path. Storage failure → `{ ok: false, error }` (shown inline next to the button). Success copy: attention *Kvitto sparat och matchat*; button *Kvitto sparat*. One receipt per purchase (*Köpet hos {leverantör} har redan ett kvitto kopplat.*).
  - **Demo** (`/demo` session file or `is_demo` business — `state.meta.demo`) → **always** inline `content_base64`, max **1,5 MB**; the bucket is never called (demo tenant ids `demo-<session>` are not uuids and demo sessions never touch Supabase). Larger → *Kvittot är för stort för demon (max 1,5 MB). Välj en mindre fil.*
