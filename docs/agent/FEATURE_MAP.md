...
- **Types:** Leverantörsfaktura (ska betalas) · Kvitto (redan betalt) · ekonomiskt dokument · **Orderbekräftelse** (`orderbekraftelse`, only when the company has sent wholesaler orders — never enters the invoice/receipt pipeline; see [Grossistbeställningar](#grossistbeställningar-wholesale-orders)). An orderbekräftelse renders `inbox-order-confirmation.tsx` (no invoice/payment fields), shows *Orderbekräftelse* → *Välj beställning* (candidates, `[data-inbox-order-candidates]` + **Det är den här**) → *Kopplad till beställning* (`[data-inbox-linked-order]`), and `/inbox/[id]/kontrollera` sends it back to `/inbox/[id]`. An unmatched one is the only wholesaler attention row: *Välj beställning för orderbekräftelsen från {avsändare}* (`inbox-mail-<id>`, CTA **Öppna i inboxen**) on Hem / Bokföring.
...
- Form (`ManualVerificationForm`): **Bokföringsdatum** (`DateField`, `min` = first day after `lockedThrough`; *Styr period, moms och räkenskapsår.* + *Bokföringen är låst till och med …*), **Handelsdatum (om annat)** (*När händelsen faktiskt inträffade. Påverkar inte perioden.*), **Vad har hänt?** (required), **Kontering** rows = `AccountCombobox` (*Välj konto*, input `aria-haspopup="listbox"`, list `role="listbox"` / `role="option"`) + **Debet** / **Kredit** inputs (`aria-label`), **Lägg till rad**, `aria-label="Ta bort raden"` (min 2 rows), live *Debet X · Kredit Y* + *N kr mer i debet/kredit*; **Underlag (rekommenderas)**: **Välj fil** / **Byt fil** (`input[type=file][accept="image/*,.pdf"]`) + **Ta bort**; **Förklaring (valfritt)**. Submit **Bokför verifikatet** (disabled until balanced and described), **Avbryt**. Success card *M{n} är bokförd på {belopp}* → **Öppna M{n}** (`/bokforing/verifikationer?v=<id>`) / **Bokför ett till**. Errors `role="alert"`.
...
| Deklarationsfil / arkiv | `/bokforing/moms` → **Hämta deklarationsfil (eSKD)** (`momsdeklaration-2026-K2.xml`); `/api/bokforing/export?typ=arkiv&ar=2026` → zip with `LÄSMIG.txt` + `underlag/register.csv` |
