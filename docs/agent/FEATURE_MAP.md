...
**Unknowns** are marked `UNKNOWN`. Facts below are from code + the live demo on 2026-09-01 unless noted. Shared address autocomplete ([PR #79](https://github.com/elialm94/driva/pull/79), hardened in [PR #94](https://github.com/elialm94/driva/pull/94): loader timeout, street-only types, Enter-while-searching) verified against main code 2026-09-04. Receipt file storage ([PR #77](https://github.com/elialm94/driva/pull/77)) verified against main code and the live demo 2026-09-04. Receipt upload hardening for **Lägg till kvitto** ([PR #90](https://github.com/elialm94/driva/pull/90): `FormData` transport, store-before-mutate, demo never touches the bucket, bucket-reject inline fallback) verified against main code 2026-09-06. Customer e-mail subjects for quotes/invoices ([PR #101](https://github.com/elialm94/driva/pull/101)) verified against main code 2026-09-04. Komplett bokföring för aktiebolag ([PR #109](https://github.com/elialm94/driva/pull/109), merge `7dcb653`: kontoregister, manuella verifikat, omvänd byggmoms, momsperiodicitet, skattekonto, lön/AGI, periodstängning, SIE-import, bokslutsbilagor, avstämning, K2-årsredovisning, INK2, inlämningslager, arkiv) verified against main code and the live demo 2026-09-05 - the new owner routes (`/bokforing/skattekonto`, `/lon`, `/verifikationer/nytt`, `/periodstangning`, `/ingaende-balans`, `/bokslut/bilagor`, `/bokslut/avstamning`, `/bokslut/ink2/<id>`) plus the eSKD, AGI and arkiv APIs answered with the demo cookie; `/bokforing/lon/[runId]` and the årsredovisning pages need data the seed does not have (no payroll run, no closed year) and the consultant routes (`/redovisning/k/...`) are code-verified only. Grossistbeställningar ([PR #111](https://github.com/elialm94/driva/pull/111), merge `d65b830`: optional feature `wholesalers`, migration `38_wholesalers`, Grossister tab, material search + cart + order e-mail, order confirmations in the Inbox) verified against main code 2026-09-06 - **code-verified only**: the live deploy checked 2026-09-06 still predates #111 and #112 (Funktioner shows only Hemsida + Samarbeta, `/api/grossist/prisfil` and `/kom-igang/importera` answer 404, `?flik=grossister` falls back to Företag instead of the Funktioner redirect), so the Grossist steps below come from code, `npx tsx --test src/lib/wholesalers.test.ts` (40/40 pass on main 2026-09-06) and the flow asserted by `scripts/verify-wholesalers-browser.ts` (read, not run for this map), not from driva-alpha. Cause, verified in CI logs 2026-09-06: main has not typechecked since `d65b830` - `src/app/api/inbox/inbound/resend/route.ts` reads `.status` / `.confirmationFollowUp` off `ingestInboundPayloadLocal(payload)` without `await` in the non-Supabase branch (`InboundWebhookResult | Promise<…>`), so `npm run typecheck` and the CI workflow fail on every main commit after #113 (later main adds an unrelated `parseReceiptDataUrl` error), and the Vercel production deploy status is *failure* for the same commits (Vercel logs not readable from here; last green deploy = #113 `b7cd39a`). Product code, not this map - remove this note once main builds again.
...
- **Series** (`src/lib/accounting/series.ts`): `A` *Löpande* (everything with underlag in the system), `M` *Manuella verifikat*. Own unbroken counters in `sequences.verificationSeries` (A mirrors the legacy `sequences.verification`); label = `${series}${number}` (`verificationLabel`) → **A36**, **M1**. Series are fixed in code, not settings. `transactionDate` (handelsdatum) is stored next to `date` (bokföringsdatum) - only `date` drives period/VAT/year.
