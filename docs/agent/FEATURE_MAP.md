...
auth.users
  └── business_memberships (role) → businesses
        ...
        ├── inbox_items (attachments JSONB; file in bucket `inbox_attachments` via storage_path, or inline ≤ 1,5 MB)
...
Also: bank (SEB …4512), expenses, supplier invoices, verifications (all serie **A**), published website, payment files. **Not seeded:** `employees`, `payrollRuns`, `employerDeclarations`, `yearEndSchedules`, `filingSubmissions`, `chartAccounts` are empty; no customer has **Omvänd byggmoms**; `vatPeriodicity` unset (= kvartal); `fSkattPerMonth` = 12 400 kr, so Skattekonto lists every past month of the year as *F-skatt att bokföra*. Fiscal year ids are per session clone (e.g. `fy-2026-…`) - read them from links, never hard-code.
