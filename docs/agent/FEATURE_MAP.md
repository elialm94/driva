...
- **Main actions — skickad:** **Skicka påminnelse** + **one** overflow (Kopiera kundlänk · Skriv ut/PDF · Öppna kundvyn; if unlinked also Starta uppdrag / Skapa faktura). No second `…` menu. When a job is already linked it lives in **Kopplat till** — do not also show Starta uppdrag / Skapa faktura. Owner dismiss from Hem: **Inte aktuell**.
- **Main actions — godkänd + kopplat uppdrag:** Primary **Fakturera** only (`createInvoiceFromQuote` = next payment-plan part; hidden when the quote is fully invoiced). **One** overflow: Kopiera kundlänk · Skriv ut/PDF · Ny version · Öppna kundvyn. No Starta uppdrag, no Öppna uppdrag, no second Skapa faktura. Right column: one **Kopplat till** card (job name + Planerat/Pågår/Klart). No **Nästa steg** card that repeats the same job or a second invoice CTA. Customer name in the subtitle is an `AppLink` to `/kunder/{id}`.
- **Main actions — godkänd utan uppdrag:** Primary **Starta uppdrag**. Overflow includes **Fakturera** (same `createInvoiceFromQuote` action) plus copy / PDF / Ny version / Öppna kundvyn. **Kopplat till:** Inte kopplat + Koppla / Skapa uppdrag (`documentLinkView` / `LinkedToBox` — do not invent new link rules).
- **Main actions — avböjd:** One overflow: Kopiera kundlänk · Skriv ut/PDF · Ny version · Öppna kundvyn.
- **Acceptance banner (owner):** Godkänd shows a **compact** banner only: *Godkänd av {namn}*, date/time, **Visa intyg** (`/offert/[token]/underlag`). No IP, user-agent, SHA-256, method essay, emails, or “Version N är låst…” paragraph in that card. Header may still show a small *Version N låst* badge. Forensics live on the intyg page / PDF.
...
- **Accept states:** success → inline *Offerten är godkänd* + receipt (who, when, amount) then `router.refresh()` → **one** server banner *Offerten är godkänd* / *Godkänd av {namn}, {tid} · {belopp}* + **Intyg om godkännande**; document shows the quiet *Godkänd {datum} av {namn}* line (not a second green box); **already accepted** = read-only, no second accept; **avbojd** / **expired** → Swedish explanation, no form; **utkast** → 404 (also for the action: `not_found`).
