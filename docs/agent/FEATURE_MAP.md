...
- **Receipt for a specific purchase (file is stored):** CTA **Lägg till kvitto** on the *Kvitto saknas – {leverantör}, {belopp}* attention item (Hem / Bokföring, `attention-list.tsx`, `cta.type === "uploadReceipt"`) or `UploadReceiptButton expenseId=…`. Client sends the `File` in a `FormData` (`receiptUploadForm`, `src/lib/receipts/read-file.ts`, max **5 MB**) — not a data-URL string (React Flight caps string action arguments). `uploadReceiptAction(form)` → `expenseAwaitingReceipt` → `storeReceiptFile` against a pre-generated receipt id → `interpretDocumentFile` → `uploadReceiptForExpense(…, { id, …stored }, interpreted)` → assert `receiptFileStored`. File is stored **before** any state mutation so a failed upload leaves *Saknar kvitto* in place. `ok: true` always means a stored file. Errors go through `userFacingStorageError` (raw RLS/Storage text never reaches the UI).
  - **Demo** (`state.meta.demo`) → always inline `content_base64`, max **1,5 MB**; the bucket is never called.
  - Real business, Supabase + service role → bucket `receipts` at `<business_id>/<receipt_id>/<filename>`; bucket rejection falls back to inline ≤ 1,5 MB.
  - JSON store or no service key → inline `content_base64`, max **1,5 MB**.
