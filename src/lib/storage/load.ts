...
export async function loadTenantState(tx: SqlExecutor, businessId: string): Promise<LoadedTenantState> {
  ...
  const [onboardingRows, dataImportRows, supplierRegisterRows, billingAllocationRows, jobChangeRows, documentLineRows] = await Promise.all([
    ...
    queryIfTable(tx, "document_lines", `select * from public.document_lines where business_id = $1 order by created_at, id`, b),
  ]);
  ...
  const state: DB = {
    ...
    documentLines: documentLineRows.map(documentLinesSpec.fromRow),
