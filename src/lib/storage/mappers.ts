...
export const requestsSpec: TableSpec<CustomerRequest> = {
  table: "requests",
  pk: ["id"],
  columns: [
    "id", "business_id", "customer_id", "title", "message", "source", "status",
    "quote_id", "idempotency_key", "notification", "ai", "created_at",
  ],
  toRow: (q, businessId) => ({
    id: q.id,
    business_id: businessId,
    ...
    idempotency_key: q.idempotencyKey ?? null,
    notification: jsonParamOrNull(q.notification),
    ai: jsonParamOrNull(q.ai),
  ...
  fromRow: (r) => ({
    ...
    source: r.source as CustomerRequest["source"],
    status: r.status as CustomerRequest["status"],
    ...opt("quoteId", strOrU(r.quote_id)),
    createdAt: tsIso(r.created_at),
    ...opt("idempotencyKey", strOrU(r.idempotency_key)),
    ...opt("notification", jsonOrU<NonNullable<CustomerRequest["notification"]>>(r.notification)),
    ...opt("ai", jsonOrU<NonNullable<CustomerRequest["ai"]>>(r.ai)),
  }),
};
...
export const quotesSpec: TableSpec<Quote & { amountToPay: number }> = {
  ...
  pk: ["id"],
  columns: [
    "id", "business_id", "number", "customer_id", "request_id", "job_id", "status",
    "current_version_id", "token", "sent_at", "viewed_at", "decided_at", "decline_reason",
    "follow_ups", "amount_to_pay", "created_at",
  ],
  toRow: (q, businessId) => ({
    id: q.id,
    business_id: businessId,
  ...
  fromRow: (r) => ({
    ...
    number: num(r.number),
    customerId: str(r.customer_id),
    ...opt("requestId", strOrU(r.request_id)),
...
export const supplierInvoicesSpec: TableSpec<SupplierInvoice> = {
  ...
  columns: [
    ...
    "vat_amount", "description", "category", "status", "bank_transaction_id",
...
export const inboxItemsSpec: TableSpec<InboxItem> = {
  ...
  pk: ["id"],
  columns: [
    "id", "business_id", "kind", "status", "external_id", "from_address", "to_address",
    "subject", "text_body", "html_body", "attachments", "parsed_amount", "parsed_vat_amount",
    "parsed_supplier", "parsed_date", "confidence", "expense_id", "created_at", "processed_at",
  ],
...
export const settingsColumns = [
  ...
  "inquiry_notification_email", "phone", "website_url", "address", "postal_code", "city",
...
export function settingsToRow(s: CompanySettings, businessId: string): Record<string, unknown> {
  return {
    ...
    email: s.email,
    inquiry_notification_email: s.inquiryNotificationEmail ?? null,
    phone: s.phone,
...
export function settingsFromRow(r: SqlRow): CompanySettings {
  return {
    ...
    email: str(r.email),
    ...opt("inquiryNotificationEmail", strOrU(r.inquiry_notification_email)),
    phone: str(r.phone),
