...
export const jobsSpec: TableSpec<Job> = {
  ...
  toRow: (j, businessId) => ({
    ...
    billing_deferrals: jsonParamOrNull(j.billingDeferrals),
    closeout: jsonParamOrNull(j.closeout),
...
export const billingAllocationsSpec: TableSpec<BillingAllocation> = {
  table: "billing_allocations",
  pk: ["id"],
  columns: [
    "id", "business_id", "job_id", "source_type", "source_id", "invoice_id", "invoice_line_id",
    "qty", "amount_excl_vat", "status", "created_at", "invoiced_at", "released_at", "release_reason",
  ],
  toRow: (a, businessId) => ({
    id: a.id,
    business_id: businessId,
    ...
    invoice_line_id: a.invoiceLineId,
    qty: a.qty ?? null,
    amount_excl_vat: a.amountExclVat,
    status: a.status,
    created_at: a.createdAt,
    invoiced_at: a.invoicedAt ?? null,
    released_at: a.releasedAt ?? null,
    release_reason: a.releaseReason ?? null,
  }),
  fromRow: (r) => ({
    id: str(r.id),
    ...opt("jobId", strOrU(r.job_id)),
    sourceType: r.source_type as BillingAllocation["sourceType"],
    sourceId: str(r.source_id),
    invoiceId: str(r.invoice_id),
    invoiceLineId: str(r.invoice_line_id),
    ...opt("qty", numOrU(r.qty)),
    amountExclVat: num(r.amount_excl_vat),
    status: r.status as BillingAllocation["status"],
    createdAt: tsIso(r.created_at),
...
export const jobChangesSpec: TableSpec<JobChange> = {
  table: "job_changes",
  pk: ["id"],
  columns: [
    "id", "business_id", "job_id", "customer_id", "number", "version", "status", "title",
    "description", "time_impact", "lines", "token", "created_at", "sent_at", "viewed_at",
    "decided_at", "decline_reason", "locked_at", "content_hash", "seller_snapshot",
    "buyer_snapshot", "approval", "replaces_change_id", "replaced_by_change_id", "created_by",
  ],
  toRow: (c, businessId) => ({
    id: c.id,
    business_id: businessId,
    ...
    time_impact: c.timeImpact ?? null,
    lines: jsonParam(c.lines),
    token: c.token,
    created_at: c.createdAt,
    sent_at: c.sentAt ?? null,
    viewed_at: c.viewedAt ?? null,
    decided_at: c.decidedAt ?? null,
    decline_reason: c.declineReason ?? null,
    locked_at: c.lockedAt ?? null,
    content_hash: c.contentHash ?? null,
    seller_snapshot: jsonParamOrNull(c.sellerSnapshot),
    buyer_snapshot: jsonParamOrNull(c.buyerSnapshot),
    approval: jsonParamOrNull(c.approval),
    replaces_change_id: c.replacesChangeId ?? null,
    replaced_by_change_id: c.replacedByChangeId ?? null,
  ...
  fromRow: (r) => ({
    ...
    status: r.status as JobChange["status"],
    title: str(r.title),
    description: str(r.description),
    ...opt("timeImpact", strOrU(r.time_impact)),
    lines: jsonVal<DocLine[]>(r.lines ?? []),
    token: str(r.token),
    createdAt: tsIso(r.created_at),
    ...opt("sentAt", tsIsoOrU(r.sent_at)),
    ...opt("viewedAt", tsIsoOrU(r.viewed_at)),
    ...opt("decidedAt", tsIsoOrU(r.decided_at)),
    ...opt("declineReason", strOrU(r.decline_reason)),
    ...opt("lockedAt", tsIsoOrU(r.locked_at)),
    ...opt("contentHash", strOrU(r.content_hash)),
    ...opt("sellerSnapshot", jsonOrU<JobChange["sellerSnapshot"]>(r.seller_snapshot)),
    ...opt("buyerSnapshot", jsonOrU<JobChange["buyerSnapshot"]>(r.buyer_snapshot)),
    ...opt("approval", jsonOrU<JobChange["approval"]>(r.approval)),
    ...opt("replacesChangeId", strOrU(r.replaces_change_id)),
    ...opt("replacedByChangeId", strOrU(r.replaced_by_change_id)),
    ...opt("createdBy", strOrU(r.created_by) as JobChange["createdBy"]),
  }),
};
...
export const invoicesSpec: TableSpec<Invoice & { amountToPay: number }> = {
  ...
  columns: [
    ...
    "issued_at", "sent_at", "last_sent_at", "last_email", "last_send_attempt_at",
    "delivered_by", "delivered_at", "paid_at", "reminders", "token", "ocr",
  ...
  fromRow: (r) => ({
    ...
    ...opt("lastSendAttemptAt", tsIsoOrU(r.last_send_attempt_at)),
    ...opt("deliveredBy", r.delivered_by == null ? undefined : (r.delivered_by as Invoice["deliveredBy"])),
    ...opt("deliveredAt", tsIsoOrU(r.delivered_at)),
...
export const wholesalerConnectionsSpec: TableSpec<WholesalerConnection> = {
  table: "wholesaler_connections",
  pk: ["id"],
  columns: [
    ...
    "customer_price_rule", "active", "active_import_id", "column_mapping", "discount_groups", "discount_agreement",
    "favorite_articles", "created_at", "updated_at",
  ],
  toRow: (c, businessId) => ({
    id: c.id,
    business_id: businessId,
    ...
    cc_self: c.ccSelf,
    default_delivery_mode: c.defaultDeliveryMode,
    default_store: c.defaultStore ?? null,
    default_delivery_address: c.defaultDeliveryAddress ?? null,
    contact_person: c.contactPerson ?? null,
    phone: c.phone ?? null,
    customer_price_rule: jsonParam(c.customerPriceRule),
    active: c.active,
    ...
    discount_agreement: jsonParamOrNull(c.discountAgreement),
  ...
  fromRow: (r) => ({
    id: str(r.id),
    wholesaler: r.wholesaler as WholesalerConnection["wholesaler"],
    ...opt("displayName", strOrU(r.display_name)),
    customerNumber: str(r.customer_number),
    orderEmail: str(r.order_email),
    ccSelf: Boolean(r.cc_self),
    defaultDeliveryMode: r.default_delivery_mode as WholesalerConnection["defaultDeliveryMode"],
    ...opt("defaultStore", strOrU(r.default_store)),
    ...opt("defaultDeliveryAddress", strOrU(r.default_delivery_address)),
    ...opt("contactPerson", strOrU(r.contact_person)),
    ...opt("phone", strOrU(r.phone)),
    customerPriceRule: jsonVal<WholesalerConnection["customerPriceRule"]>(r.customer_price_rule ?? { kind: "later" }),
    ...
    ...opt("discountAgreement", jsonOrU<WholesalerConnection["discountAgreement"]>(r.discount_agreement)),
...
export const wholesalerPriceImportsSpec: TableSpec<WholesalerPriceImport> = {
  table: "wholesaler_price_imports",
  pk: ["id"],
  columns: [
    "id", "business_id", "connection_id", "filename", "file_kind", "format", "status", "mapping", "row_count",
    "product_count", "skipped_count", "errors", "has_article_register", "has_discounts",
    "discount_group_count", "price_date", "failed_reason", "created_at", "completed_at",
  ],
  toRow: (i, businessId) => ({
    id: i.id,
    business_id: businessId,
    ...
    filename: i.filename,
    file_kind: i.fileKind,
    format: i.format ?? null,
    status: i.status,
    mapping: jsonParam(i.mapping),
    row_count: i.rowCount,
    product_count: i.productCount,
    skipped_count: i.skippedCount,
    errors: jsonParam(i.errors),
    has_article_register: i.hasArticleRegister,
    has_discounts: i.hasDiscounts,
    discount_group_count: i.discountGroupCount,
    price_date: i.priceDate,
    failed_reason: i.failedReason ?? null,
  ...
  fromRow: (r) => ({
    ...
    filename: str(r.filename),
    fileKind: r.file_kind as WholesalerPriceImport["fileKind"],
    ...opt("format", strOrU(r.format) as WholesalerPriceImport["format"]),
    status: r.status as WholesalerPriceImport["status"],
    mapping: jsonVal<WholesalerPriceImport["mapping"]>(r.mapping ?? {}),
    rowCount: num(r.row_count),
    productCount: num(r.product_count),
    skippedCount: num(r.skipped_count),
    errors: jsonVal<WholesalerPriceImport["errors"]>(r.errors ?? []),
    hasArticleRegister: Boolean(r.has_article_register),
    hasDiscounts: Boolean(r.has_discounts),
    discountGroupCount: num(r.discount_group_count),
    priceDate: dateOnly(r.price_date),
    ...opt("failedReason", strOrU(r.failed_reason)),
    createdAt: tsIso(r.created_at),
    ...opt("completedAt", tsIsoOrU(r.completed_at)),
  }),
};
...
export const purchaseOrdersSpec: TableSpec<PurchaseOrder> = {
  table: "purchase_orders",
  pk: ["id"],
  columns: [
    "id", "business_id", "reference", "job_id", "connection_id", "status", "channel", "delivery",
    "orderer_name", "orderer_email", "orderer_phone", "message", "cc_self", "wholesaler_order_number",
    "sent_at", "sent_snapshot", "last_email", "last_send_attempt_at", "send_key", "created_by_user_id",
    "cancelled_at", "deviations_accepted_at", "created_at", "updated_at",
  ],
  toRow: (o, businessId) => ({
    id: o.id,
    business_id: businessId,
    ...
    status: o.status,
    channel: o.channel,
    delivery: jsonParam(o.delivery),
    orderer_name: o.ordererName,
    orderer_email: o.ordererEmail,
    orderer_phone: o.ordererPhone,
    message: o.message ?? null,
    cc_self: o.ccSelf,
    wholesaler_order_number: o.wholesalerOrderNumber ?? null,
    sent_at: o.sentAt ?? null,
    sent_snapshot: jsonParamOrNull(o.sentSnapshot),
    last_email: jsonParamOrNull(o.lastEmail),
    last_send_attempt_at: o.lastSendAttemptAt ?? null,
    send_key: o.sendKey ?? null,
    // Demosessioner har icke-uuid-användare – kolumnen är uuid, så bara riktiga id:n skrivs.
    created_by_user_id: o.createdByUserId && UUID_RE.test(o.createdByUserId) ? o.createdByUserId : null,
    cancelled_at: o.cancelledAt ?? null,
    deviations_accepted_at: o.deviationsAcceptedAt ?? null,
    created_at: o.createdAt,
    updated_at: o.updatedAt,
  }),
  fromRow: (r) => ({
    ...
    channel: (strOrU(r.channel) as PurchaseOrder["channel"] | undefined) ?? "email",
    delivery: jsonVal<PurchaseOrder["delivery"]>(r.delivery ?? { mode: "pickup" }),
    ordererName: str(r.orderer_name),
    ordererEmail: str(r.orderer_email),
    ordererPhone: str(r.orderer_phone),
    ...opt("message", strOrU(r.message)),
    ccSelf: Boolean(r.cc_self),
    ...opt("wholesalerOrderNumber", strOrU(r.wholesaler_order_number)),
    ...opt("sentAt", tsIsoOrU(r.sent_at)),
    ...opt("sentSnapshot", jsonOrU<PurchaseOrder["sentSnapshot"]>(r.sent_snapshot)),
    ...opt("lastEmail", jsonOrU<PurchaseOrder["lastEmail"]>(r.last_email)),
    ...opt("lastSendAttemptAt", tsIsoOrU(r.last_send_attempt_at)),
    ...opt("sendKey", strOrU(r.send_key)),
    ...opt("createdByUserId", strOrU(r.created_by_user_id)),
    ...opt("cancelledAt", tsIsoOrU(r.cancelled_at)),
    ...opt("deviationsAcceptedAt", tsIsoOrU(r.deviations_accepted_at)),
    createdAt: tsIso(r.created_at),
    updatedAt: tsIso(r.updated_at),
  }),
};
...
export const purchaseOrderLinesSpec: TableSpec<PurchaseOrderLine> = {
  table: "purchase_order_lines",
  pk: ["id"],
  columns: [
    "id", "business_id", "order_id", "position", "product_id", "article_number", "name", "e_number",
    "rsk_number", "unit", "pack_size", "qty", "unit_cost_ore", "customer_unit_price_ore",
    "customer_price_source", "note", "is_free_text", "job_work_entry_id", "created_at", "updated_at",
  ],
  toRow: (l, businessId) => ({
    id: l.id,
    business_id: businessId,
    ...
    qty: l.qty,
    unit_cost_ore: l.unitCostOre ?? null,
    customer_unit_price_ore: l.customerUnitPriceOre ?? null,
    customer_price_source: l.customerPriceSource,
    note: l.note ?? null,
    is_free_text: l.isFreeText,
    job_work_entry_id: l.jobWorkEntryId ?? null,
  ...
  fromRow: (r) => ({
    ...
    position: num(r.position),
    ...opt("productId", strOrU(r.product_id)),
    ...
    qty: num(r.qty),
    ...opt("unitCostOre", numOrU(r.unit_cost_ore)),
    ...opt("customerUnitPriceOre", numOrU(r.customer_unit_price_ore)),
    customerPriceSource: r.customer_price_source as PurchaseOrderLine["customerPriceSource"],
    ...opt("note", strOrU(r.note)),
    isFreeText: Boolean(r.is_free_text),
    ...opt("jobWorkEntryId", strOrU(r.job_work_entry_id)),
    createdAt: tsIso(r.created_at),
    updatedAt: tsIso(r.updated_at),
  }),
};
...
export const purchaseOrderConfirmationsSpec: TableSpec<PurchaseOrderConfirmation> = {
  table: "purchase_order_confirmations",
  pk: ["id"],
  columns: [
    "id", "business_id", "order_id", "inbox_item_id", "source", "match_method", "status", "received_at",
    "wholesaler_order_number", "delivery_date", "total_ore", "message", "lines", "deviations",
  ...
  toRow: (c, businessId) => ({
    ...
    business_id: businessId,
    order_id: c.orderId,
    inbox_item_id: c.inboxItemId ?? null,
    source: c.source,
    match_method: c.matchMethod,
    status: c.status,
    received_at: c.receivedAt,
    wholesaler_order_number: c.wholesalerOrderNumber ?? null,
    delivery_date: c.deliveryDate ?? null,
    total_ore: c.totalOre ?? null,
    message: c.message ?? null,
    lines: jsonParam(c.lines),
    deviations: jsonParam(c.deviations),
  ...
  fromRow: (r) => ({
    ...
    orderId: str(r.order_id),
    ...opt("inboxItemId", strOrU(r.inbox_item_id)),
    source: r.source as PurchaseOrderConfirmation["source"],
    matchMethod: r.match_method as PurchaseOrderConfirmation["matchMethod"],
    status: r.status as PurchaseOrderConfirmation["status"],
    receivedAt: tsIso(r.received_at),
    ...opt("wholesalerOrderNumber", strOrU(r.wholesaler_order_number)),
    ...opt("deliveryDate", dateOnlyOrU(r.delivery_date)),
    ...opt("totalOre", numOrU(r.total_ore)),
    ...opt("message", strOrU(r.message)),
    lines: jsonVal<PurchaseOrderConfirmation["lines"]>(r.lines ?? []),
    deviations: jsonVal<PurchaseOrderConfirmation["deviations"]>(r.deviations ?? []),
    ...opt("reviewedAt", tsIsoOrU(r.reviewed_at)),
    createdAt: tsIso(r.created_at),
  }),
};
