...
/**
 * Svenskt företag med adress och betalning kvar att fylla i. Momsreg.nr är
 * inte med: det härleds ur org.nr och kan därför aldrig saknas på egen hand.
 */
...
/** Utländskt bolag: momsnummer är ett manuellt fält som kan saknas. */
function foreignBusiness(over: Partial<ReturnType<typeof testCompany>> = {}) {
  return testCompany({ country: "Tyskland", vatNumber: "", ...over });
}
...
describe("Företags- vs dokumentblockers", () => {
  it("räknar bara företagsnivå i Inställningar – inte kund, rad eller ROT", () => {
    ...
    assert.deepEqual(business.map((b) => b.code).sort(), ["seller_address", "seller_bankgiro"]);
  ...
  it("settingsBillingReadiness använder samma koder som collectSellerBlockers", () => {
    ...
    assert.deepEqual(
      ...
      ["Företagsadress", "Betalningsuppgifter"]
    );
    assert.deepEqual(readiness.previewLabels, ["Företagsadress", "Betalningsuppgifter"]);
...
describe("Fakturering-status: konsekvens och copy", () => {
  it("säger att fakturor inte kan skickas när företagsblockers finns", () => {
    ...
    assert.equal(readiness.consequence, "2 uppgifter behöver kompletteras innan du kan skicka fakturor.");
  ...
  it("nämner inte offerter när bara betalning saknas", () => {
...
describe("Fylla i en i taget", () => {
  it("räknar ner 2 → 1 → redo när adress och bankgiro fylls i", () => {
    let seller = incompleteBusiness();
    assert.equal(settingsBillingReadiness(seller).missingCount, 2);
    ...
    assert.equal(afterAddress.missingCount, 1);
    assert.deepEqual(
      afterAddress.items.map((i) => i.label),
      ["Betalningsuppgifter"]
    );
    assert.equal(afterAddress.items[0].hint, "Lägg till minst ett betalningssätt.");
...
describe("Momsreg.nr blockerar inte svenska företag", () => {
  it("ett svenskt företag med giltigt org.nr saknar aldrig momsreg.nr", () => {
    const codes = collectSellerBlockers(testCompany({ vatNumber: "" })).map((b) => b.code);
    assert.ok(!codes.includes("seller_vat"));
    assert.ok(!codes.includes("seller_vat_format"));
  });

  it("ett felaktigt sparat momsnummer blockerar inte – det härledda används", () => {
    const codes = collectSellerBlockers(testCompany({ vatNumber: "SE000000000001" })).map((b) => b.code);
    assert.equal(codes.length, 0);
  });

  it("saknat org.nr ger bara org.nr-blockern, inte också en moms-blocker", () => {
    const codes = collectSellerBlockers(testCompany({ orgNumber: "", vatNumber: "" })).map((b) => b.code);
    assert.deepEqual(codes, ["seller_orgnr"]);
  });

  it("utländskt bolag måste fortfarande fylla i sitt momsnummer", () => {
    assert.ok(collectSellerBlockers(foreignBusiness()).some((b) => b.code === "seller_vat"));
    assert.equal(collectSellerBlockers(foreignBusiness({ vatNumber: "DE123456789" })).length, 0);
    assert.ok(
      collectSellerBlockers(foreignBusiness({ vatNumber: "12345" })).some((b) => b.code === "seller_vat_format")
    );
  });
});

describe("Deeplänkar till rätt fält", () => {
  it("pekar adress och betalning till rätt flik och fält", () => {
  ...
  it("utländskt momsnummer pekar till momsfältet på företagsfliken", () => {
    const items = groupBusinessBlockers(collectSellerBlockers(foreignBusiness()));
...
describe("Efter komplettering: faktura och offert", () => {
  ...
  it("tar bort företagsblockers från fakturans send-lista när uppgifterna fylls i", () => {
    ...
    assert.equal(billingReadiness().missingCount, 2);
