 ...
 * Vyn visar varje balanskonto vid sidan av sitt underlag och pekar på
 * skillnaden när den finns. Samma vy på ägarytan och konsultytan.
 */

/** Året avstämningen gäller: det äldsta öppna. */
export function avstamningFiscalYear() {
  return fiscalYears().filter((f) => f.status === "oppet")[0];
}

export function AvstamningView({ hrefFor = (href: string) => href }: { hrefFor?: (ownerHref: string) => string }) {
                ...
                <td className="py-2 pr-3 align-top">
                  ...
                  <span className="mt-0.5 block pl-5 text-[12px] text-muted">
                    ...
                    {!row.ok && row.href ? (
                      <>
                        {" "}
                        <Link href={hrefFor(row.href) as never} className="font-medium text-accent hover:underline">
                          {row.hrefLabel ?? "Visa"}
                        </Link>
                      </>
                    ) : null}
