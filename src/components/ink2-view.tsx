 ...
 *
 * Vyn visar varför bokföringens resultat och skattens resultat är olika tal.
 ...
 *
 * Samma vy på ägarytan och konsultytan – konsulten upprättar deklarationen.
 */

export interface Ink2ViewProps {
  fy: FiscalYear;
  /** Klienten deklarationen gäller. Konsultytan skickar den. */
  businessId?: string;
  /** Revisorn läser beräkningen men lämnar inte in den. */
  readOnly?: boolean;
}

/** Är bolagsformen en annan än aktiebolag finns ingen INK2 att visa. */
export function ink2Applies(): boolean {
  return (db().settings.companyForm ?? "ab") === "ab";
}

export function Ink2View({ fy, businessId, readOnly }: Ink2ViewProps) {
  ...
  return (
    <>
      ...
      <SruCard fiscalYearId={fy.id} label={fy.label} businessId={businessId} readOnly={readOnly} />
...
function SruCard({
  ...
  return (
    <Card className="mb-6 px-6 py-5">
      ...
      {readOnly ? null : (
        <InlamningPanel {...filingPanelData("ink2", fiscalYearId)} businessId={businessId} className="mt-4" />
      )}
    </Card>
  );
}
