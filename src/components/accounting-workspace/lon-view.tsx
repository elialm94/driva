...
import { isOwnerSurface, wsCan, type AccountingWorkspace } from "@/lib/accounting-workspace/shared";
import {
  employeeById,
  employees,
...
/**
 * Lönevyn – samma på ägar- och konsultytan. Skrivknapparna kräver
 * write_accounting; personnumret avslöjas bara med reveal_personnummer.
 */
export function LonView({ ws }: { ws: AccountingWorkspace }) {
  const canWrite = wsCan(ws, "write_accounting");
  const canReveal = wsCan(ws, "reveal_personnummer");
      ...
      {allEmployees.length > 0 ? (
        <>
          <div className="mb-4 space-y-4">
                        ...
                        <div>
                          ...
                          <dd>
                            {canReveal ? (
                              ...
                              <span className="tabular">{maskPersonnummer(person.personnummer)}</span>
                            )}
                          </dd>
                        </div>
                        <div>
            ...
            {canWrite ? <EmployeeForm today={today} /> : null}
          </div>
          ...
          <div className="space-y-3">
            {declarations.map((d) => (
              <Card key={d.id} className="px-6 py-4">
                ...
                {canWrite ? <InlamningPanel {...filingPanelData("agi", d.month)} className="mt-3" /> : null}
              </Card>
            ))}
          </div>
        )}
        {missingDraft.length > 0 && canWrite ? (
          <div className="mt-3">
