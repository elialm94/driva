...
/** Saldobalansen – samma vy på ägar- och konsultytan. */
export function SaldobalansView({ ws, searchParams: params }: { ws: AccountingWorkspace; searchParams: { ar?: string } }) {
  const { years, fy, saldo: sb } = rapporterViewModel(params);
  const activeLabel = fy.label;
  const base = wsHref(ws, "/bokforing/saldobalans");
