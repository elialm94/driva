...
/** Ett underlag i detalj – samma vy på ägar- och konsultytan. */
export function UnderlagDetailView({ ws, id }: { ws: AccountingWorkspace; id: string }) {
  const view = getInboxView(id);
  if (!view) {
    const job = getJob(id);
    if (job && isOwnerSurface(ws)) redirect(`/uppdrag/${job.id}`);
    notFound();
  }
  const canPay = wsCan(ws, "prepare_supplier_payment");
  const canWrite = wsCan(ws, "write_accounting");
  const href = (ownerHref: string) => wsHref(ws, ownerHref);
    ...
    return (
      <div className="animate-fade-up">
        <PageHeader
          ...
          crumbs={[{ href: href(kunderInboxHref()), label: "Underlag" }, { label: item.subject || "Orderbekräftelse" }]}
          ...
          actions={canWrite ? <InboxOverflowMenu itemId={item.id} canIgnore={item.status === "ny"} /> : undefined}
        />
  ...
  return (
    <div className="animate-fade-up">
      <PageHeader
        back={isOwnerSurface(ws) ? <SmartBack /> : undefined}
        crumbs={[
          { href: href(kunderInboxHref()), label: "Underlag" },
        ...
        actions={
          <div className="flex items-center gap-2">
            {amountReview && canWrite ? (
              <ButtonLink href={href(`/bokforing/underlag/${item.id}/kontrollera`)} size="sm">
                Kontrollera belopp
              </ButtonLink>
            ) : canCreateFile && invoice && canPay ? (
            ...
            {canWrite ? <InboxOverflowMenu itemId={item.id} canIgnore={item.status === "ny"} /> : null}
          </div>
        }
      />
          ...
          {paymentFile && payment?.status === "PAYMENT_FILE_CREATED" ? (
            <Card className="border-ok/30 bg-ok-soft/30 p-5">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                ...
                {canPay ? <RegeneratePaymentFileButton fileId={paymentFile.id} /> : null}
              </div>
            </Card>
          ) : null}

          {detailsPanel && canPay ? <SupplierPaymentDetailsPanel {...detailsPanel} /> : null}
