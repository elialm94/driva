...
import type { BalansRad } from "@/lib/accounting/ledger";
...
import { rapporterViewModel } from "@/lib/accounting-workspace/view-models";
...
/** Balansrapporten – samma vy på ägar- och konsultytan. */
export function BalansView({ ws, searchParams: params }: { ws: AccountingWorkspace; searchParams: { ar?: string } }) {
  const { years, fy, balans: br } = rapporterViewModel(params);
  const base = wsHref(ws, "/bokforing/balans");
  ...
  return (
    <div>
      ...
      <FiscalYearPicker
        ...
        hrefFor={(y) => fiscalYearHref(base, y)}
      />
