...
import { huvudbokViewModel } from "@/lib/accounting-workspace/view-models";
import { isOwnerSurface, wsHref, type AccountingWorkspace } from "@/lib/accounting-workspace/shared";

/** Radgräns per sida i kontodetaljen – huvudboken kan ha tiotusentals rader. */
const PAGE_SIZE = 200;

/** Huvudboken – samma vy på ägar- och konsultytan. */
export function HuvudbokView({
  ws,
  searchParams: params,
}: {
  ws: AccountingWorkspace;
  searchParams: { konto?: string; sida?: string; ar?: string };
}) {
  const { years, fy, selected, accounts, account } = huvudbokViewModel(params);
  const extra = selected ? { konto: String(selected) } : undefined;
  const base = wsHref(ws, "/bokforing/huvudbok");
  ...
  return (
    <div>
      ...
      <FiscalYearPicker
        ...
        hrefFor={(y) => fiscalYearHref(base, y, extra)}
      />
      ...
      <div className="mb-6 flex flex-wrap gap-1.5 print:hidden">
        ...
        {accounts.map((a) => (
          <Link
            key={a.account}
            href={fiscalYearHref(base, fy, { konto: String(a.account) })}
            className={cx(
      ...
      {accounts.length === 0 ? (
                ...
                return (
                  <tr key={a.account} className="border-t border-line/50">
                    <td className="py-1.5 pr-3">
                      <Link href={fiscalYearHref(base, fy, { konto: String(a.account) })} className="hover:underline">
        ...
        (() => {
          ...
          const pageHref = (p: number) =>
            fiscalYearHref(base, fy, { konto: String(account.account), sida: String(p) });
          return (
