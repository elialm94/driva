 ...
 */
export function LonespecifikationView({ ws, runId }: { ws: AccountingWorkspace; runId: string }) {
        ...
        <div className="flex flex-wrap items-start justify-between gap-6">
          ...
          <div>
            ...
            <p className="text-[13px] text-soft">
              {wsCan(ws, "reveal_personnummer") ? (
                <RevealPersonnummer employeeId={employee.id} masked={maskPersonnummer(employee.personnummer)} />
              ) : (
                <span className="tabular">{maskPersonnummer(employee.personnummer)}</span>
              )}
            </p>
      ...
      {verification ? (
        <p className="text-[12px] text-muted">
          ...
          <Link
            ...
            href={wsHref(ws, "/bokforing/verifikationer")}
          >
