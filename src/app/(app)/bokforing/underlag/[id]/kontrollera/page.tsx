...
export default async function KontrolleraPage(props: { params: Promise<{ id: string }> }) {
  ...
  if (!review.editable || item.documentType === "orderbekraftelse") redirect(`/bokforing/underlag/${id}`);
  ...
  return (
    <div className="animate-fade-up">
      <PageHeader
        back={<SmartBack />}
        crumbs={[
          { href: kunderInboxHref(), label: "Underlag" },
          { href: `/bokforing/underlag/${item.id}`, label: who },
          { label: "Kontrollera" },
        ]}
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(360px,1fr)] lg:items-start">
        ...
        <ExtractionReviewForm
          ...
          backHref={`/bokforing/underlag/${item.id}`}
        />
      </div>
    </div>
  );
