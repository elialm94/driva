...
- **No em dash and no en dash in prose.** Use a hyphen, or restructure the sentence. Enforced by
  `src/lib/em-dash-scan.test.ts`, which fails on a dash with whitespace on either side inside a
  string literal or JSX text under `src/`. An em dash (U+2014) fails anywhere under `src/`; an en
  dash (U+2013) fails in the bookkeeping surface (`src/lib/accounting/`, `src/lib/bas.ts`,
  `src/lib/ai/accounting-domain.ts`), listed as `BOKFORING` in the guard. The rest of `src/` still
  carries en dashes in prose and is cleaned up separately, one area at a time; widen `BOKFORING` as
  each area is cleaned. The standalone table placeholder `"—"` (a dash that is the whole string,
  meaning "no value") is correct typography, is exempt, and must keep working; the same goes for
  `"–"`. Comments are outside the guard; do not rewrite existing ones on your own initiative.
